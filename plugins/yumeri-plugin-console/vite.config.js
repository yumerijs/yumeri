...
function clientPrebundlePlugin() {
  return {
    name: 'yumeri-console-client-prebundle',
    async closeBundle() {
...
function buildTailwindCss() {
  const input = path.resolve(projectRoot, 'src/styles/tailwind.css')
  const output = path.join(clientDir, 'shadcn.css')
  const cli = require.resolve('tailwindcss/lib/cli.js')
  fs.mkdirSync(clientDir, { recursive: true })
  execSync(`node ${cli} -i ${input} -o ${output} --minify`, {
    cwd: projectRoot,
    stdio: 'inherit',
  })
}
